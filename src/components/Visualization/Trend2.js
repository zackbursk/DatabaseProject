...
const Trend2 = () => {
  ...
  const [airportName,setGenre]=useState("Orlando International Airport")
  ...
  function queryData()
  {
    axios.get(`http://localhost:5000/api/trend2/${airportName}/${year}`).then(res => {
    ...
    return (
        ...
        <div>
          <h1 className="app-h11">Delays Based On Different Airports</h1>
       </div>
       <div className="chart">
        <Line 
        data={{
      ...
      />
        ...
        <form>
            <label>
              <p>Airport Name: </p>
            <input type="text" name="Genre" value={airportName} onChange={handleGenreChange}/>
            </label>
            <label>
            <p>Year (2010 - 2014): </p>
          ...
          <NavLink className="nav-link2" to="/Trend7">Over all years</NavLink>
        </div>
        </header>
      </div>
