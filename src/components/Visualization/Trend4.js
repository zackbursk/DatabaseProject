...
const Trend4 = () => {
  ...
  const [cityCode,setName]=useState("LAS")
  ...
  function queryData()
  {
    axios.get(`http://localhost:5000/api/trend4/${cityCode}/${year}`).then(res => {
    ...
    return (
      ...
      />
        ...
        <form>
            <label>
            <p>Airport City Code: </p>
            <input type="text" name="Name" value={cityCode} onChange={handleNameChange}/>
            </label>
            <label>
            <p>Year (2010 - 2014): </p>
          ...
          <NavLink className="nav-link2" to="/Trend9">Over all years</NavLink>
        </div>
        </header>
      </div>
