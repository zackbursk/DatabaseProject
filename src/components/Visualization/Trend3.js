...
const Trend3 = () => {
  ...
  const [airplaneName,setName]=useState("Boeing 737-800")
  ...
  function queryData()
  {
    axios.get(`http://localhost:5000/api/trend3/${airplaneName}/${year}`).then(res => {
      console.log("here");
    ...
    return (
      ...
      <div>
        <h1 className="app-h11">Delays Based On Different Models of Airplanes</h1>
     </div>
     <div className="chart">
        <Line 
        data={{
      ...
      />
        ...
        <form>
            <label>
            <p>Airplane Name: </p>
            <input type="text" name="Name" value={airplaneName} onChange={handleNameChange}/>
            </label>
            <label>
            <p>Year (2010 - 2014): </p>
          ...
          <NavLink className="nav-link2" to="/Trend8">Over all years</NavLink>
        </div>
        </header>
      </div>
